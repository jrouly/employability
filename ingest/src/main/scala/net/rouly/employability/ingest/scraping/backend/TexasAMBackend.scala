...
class TexasAMBackend(
  jsoup: JSoupClient
)(implicit ec: BlockingExecutionContext)
  extends GenericBackend(jsoup) {

  private val baseUrl = "http://catalog.tamu.edu"

  override protected val dataSet = "Texas A and M"

  protected def getDepartmentUrls: Future[List[Url]] = {
    jsoup.get(baseUrl / "/undergraduate/course-descriptions").map { document =>
      document
        .select("#atozindex")
        .select("li")
        .select("a")
